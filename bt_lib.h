...
typedef struct {
  ...
  char bitfield[100]; //bitfield where each bit represents a piece that
...
typedef struct bt_msg{
  ...
  char data[8192];//pointer to start of payload, just incase
...
void intiateExchange(int sockfd, int isServer, bt_info_t * bt_info,char clntName[],unsigned short ownPort,char mdString1[],bt_args_t * bt_args);
