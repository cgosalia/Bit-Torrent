...
void parse_args(bt_args_t * bt_args, int argc,  char * argv[]){
  ...
  while ((ch = getopt(argc, argv, "hp:s:l:vIb")) != -1) {
    switch (ch) {
    case 'h': //help
      usage(stdout);
      exit(0);
    ...
    case 'b':
	bt_args->connectip = 1;
	//strncpy(bt_args->connectip,optarg,BUFFERSIZE);
	break;
    case 'p': //peer
      n_peers++;
  ...
  if(strcmp(bt_args->log_file,"") != 0)
    log_ptr.logging_file = fopen(bt_args->log_file,"w");
